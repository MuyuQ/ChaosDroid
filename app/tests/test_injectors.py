...
class TestLowBatteryInjector:
    ...
    def low_battery_injector(self):
        """创建低电量注入器实例。"""
        from app.injectors.low_battery import LowBatteryInjector
...
class TestNetworkJitterInjector:
    ...
    def network_injector(self):
        """创建网络波动注入器实例。"""
        from app.injectors.network_jitter import NetworkJitterInjector
...
class TestRebootTimeoutInjector:
    ...
    def reboot_injector(self):
        """创建重启超时注入器实例。"""
        from app.injectors.reboot_timeout import RebootTimeoutInjector
...
class TestCpuIoStressInjector:
    ...
    def cpu_io_injector(self):
        """创建CPU/I/O压力注入器实例。"""
        from app.injectors.cpu_io_stress import CpuIoStressInjector
...
class TestMonkeyStabilityInjector:
    ...
    def monkey_injector(self):
        """创建Monkey稳定性注入器实例。"""
        from app.injectors.monkey_stability import MonkeyStabilityInjector
