...
class RebootTimeoutInjector(BaseInjector):
    ...
    def __init__(self):
        self.timeout_duration_sec: int = 60
        self.boot_delay_sec: int = 30
    ...
    async def prepare(self, context: InjectContext) -> bool:
        ...
        self.timeout_duration_sec = params.get("timeout_duration_sec", 60)
        self.boot_delay_sec = params.get("boot_delay_sec", 30)
    ...
    async def inject(self, context: InjectContext) -> InjectResult:
        ...
        if is_mock:
            ...
            state.apply_injection("reboot_timeout", {"boot_delay": self.boot_delay_sec})
            ...
            return InjectResult(
                ...
                details={
                    "timeout_duration_sec": self.timeout_duration_sec,
    ...
    async def cleanup(self, context: InjectContext) -> bool:
        ...
        # 等待boot完成
        boot_completed = await executor.wait_for_boot(timeout=self.timeout_duration_sec + 30)
