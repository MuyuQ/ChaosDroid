"""CPU/I/O压力注入器."""
import asyncio
import random
from typing import Dict, Any, List

from chaosdroid.injectors.base import (
    BaseInjector,
    InjectContext,
    InjectResult
)
from chaosdroid.models.base import FaultType, RiskLevel


def _validate_path(path: str) -> bool:
    """验证路径是否安全，防止命令注入."""
    # 只允许安全字符：字母、数字、下划线、斜杠、点
    import re
    safe_pattern = r'^[a-zA-Z0-9_/.-]+$'
    if not re.match(safe_pattern, path):
        return False
    # 不允许路径遍历
    if '..' in path:
        return False
    # 路径必须以 /sdcard 或 /data 开始（Android安全路径）
    if not path.startswith('/sdcard/') and not path.startswith('/data/'):
        return False
    return True


class CpuIoStressInjector(BaseInjector):
    """CPU/I/O压力注入器

    在设备上启动压力任务，模拟高负载场景。
    每次注入调用独立，不维护实例状态。
    """

    fault_type = FaultType.cpu_io_stress
    risk_level = RiskLevel.medium

    async def prepare(self, context: InjectContext) -> bool:
        """准备注入环境"""
        executor = context.executor
        fault_profile = context.fault_profile

        # 获取压力参数（每次调用重新获取）
        params = fault_profile.get("parameters", {})
        cpu_load_percent = params.get("cpu_load_percent", 50)
        io_enabled = params.get("io_enabled", True)
        duration_sec = params.get("duration_sec", 60)

        # 检查设备在线
        online = await executor.is_online()
        if not online:
            return False

        return True

    async def inject(self, context: InjectContext) -> InjectResult:
        """执行CPU/I/O压力注入"""
        executor = context.executor
        fault_profile = context.fault_profile

        # 每次调用重新获取参数（不使用实例状态）
        params = fault_profile.get("parameters", {})
        cpu_load_percent = params.get("cpu_load_percent", 50)
        io_enabled = params.get("io_enabled", True)
        duration_sec = params.get("duration_sec", 60)

        # 使用本地变量跟踪进程状态
        stress_processes: List[str] = []

        # 检查是否是Mock设备
        is_mock = hasattr(executor, 'get_state')
        if is_mock:
            # Mock模式：添加压力进程状态
            state = executor.get_state()
            state.apply_injection("cpu_io_stress", {
                "cpu_load": cpu_load_percent,
                "io_enabled": io_enabled
            })

            await asyncio.sleep(random.uniform(0.5, 1.0))

            # 模拟运行一段时间
            await asyncio.sleep(min(duration_sec, 5))

            return InjectResult(
                success=True,
                fault_type=self.fault_type,
                fault_injected=True,
                fault_observed=True,
                message=f"Mock注入: CPU负载={cpu_load_percent}%, I/O={io_enabled}",
                details={
                    "cpu_load_percent": cpu_load_percent,
                    "io_enabled": io_enabled,
                    "duration_sec": duration_sec,
                    "stress_processes": len(state.stress_processes)
                },
                cleanup_required=True
            )
        else:
            # Real模式：启动压力任务
            # CPU压力 - 使用数值参数（安全）
            cpu_cmd = f"timeout {int(duration_sec)} yes > /dev/null"
            result = await executor.execute_shell(cpu_cmd, timeout=int(duration_sec) + 10)

            if result.success or result.exit_code == 124:  # timeout正常退出
                stress_processes.append("cpu_stress")

            # I/O压力 - 使用验证过的安全路径
            io_path = "/sdcard/stress_io.dat"
            if io_enabled:
                # 验证路径安全性
                if _validate_path(io_path):
                    io_cmd = f"timeout {int(duration_sec)} dd if=/dev/zero of='{io_path}' bs=1M count=100"
                    result = await executor.execute_shell(io_cmd, timeout=int(duration_sec) + 10)

                    if result.success or result.exit_code == 124:
                        stress_processes.append("io_stress")

            success = len(stress_processes) > 0

            return InjectResult(
                success=success,
                fault_type=self.fault_type,
                fault_injected=success,
                fault_observed=True,
                message=f"真实模式: 启动{len(stress_processes)}个压力任务",
                details={
                    "cpu_load_percent": cpu_load_percent,
                    "io_enabled": io_enabled,
                    "duration_sec": duration_sec,
                    "stress_processes": stress_processes,
                    "real_mode": True
                },
                cleanup_required=True
            )

    async def cleanup(self, context: InjectContext) -> bool:
        """清理注入"""
        executor = context.executor

        # 检查是否是Mock设备
        is_mock = hasattr(executor, 'get_state')
        if is_mock:
            # Mock模式：清除压力进程状态
            state = executor.get_state()
            state.apply_recovery("stop_stress")
            return True

        # Real模式：停止压力任务
        # 停止yes进程（不涉及路径参数，相对安全）
        result = await executor.execute_shell("pkill -f 'yes > /dev/null'", timeout=10)

        # 清理I/O文件（使用硬编码安全路径）
        io_path = "/sdcard/stress_io.dat"
        if _validate_path(io_path):
            result = await executor.execute_shell(f"rm -f '{io_path}'", timeout=10)

        return True


# 注册注入器
from chaosdroid.injectors.base import register_injector
register_injector(CpuIoStressInjector())